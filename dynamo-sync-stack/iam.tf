resource "aws_iam_role" "ddb-stream-lambda-executor" {
  name               = "${var.table.name}-DynamoDBStreamLambdaExecutioner"
...
resource "aws_iam_policy" "ddb-stream-lambda-executor-policy" {
  name   = "${var.table.name}-DynamoDBStreamLambdaExecutionerPolicy"
  policy = jsonencode({
    Version   = "2012-10-17",
    Statement = [
      {
...
resource "aws_iam_policy_attachment" "ddb-stream-lambda-executor-policy-attachment" {
  name       = "${var.table.name}-ddb-stream-lambda-executor-policy-attachment"
